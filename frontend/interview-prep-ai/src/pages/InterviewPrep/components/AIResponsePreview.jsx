    ...
    return (
        <div className="max-w-4xl mx-auto">
            <div className="text-[14px] prose prose-slate dark:prose-invert max-w-none">
                {/* the AI API will return a markdown text which we are formatting as we want using react markdown */}
                <ReactMarkdown 
                    remarkPlugins={[remarkGfm]}
                    /* components are the different types of styles found in the markdown text */
                    components={{
                        // for text marked as code in the markdown, we will apply the follwing custom styles
                        code({node, className, children, ...props}) {
                            ...
                            return !isInline ? (
                                ...
                                <code className="px-1 py-0.5 bg0gray-100 rounded text-sm" {...props}>
                                    {children}
                                </code>                                
                            )
                        },
                        ...
                        img({src, alt}) {
                            return <img src={src} alt={alt} className="my-4 max-w-full rounded">{children}</img>
                        },
                    }}
                >
                    {content}
...
//function to style code blocks in a special way using syntax highlighter
function CodeBlock({code, language}) {
    ...
    return <div className="relative my-6 rounded-lg overflow-hidden bg-gray-50 border border-gray-200" >
        ...
        {/* styles the actual code block in special colors making it stand out and easy to read */}
        <SyntaxHighlighter
            language={language}
            style={oneLight}
